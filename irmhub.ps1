...
$script:CATALOG = @(
    ...
    [PSCustomObject]@{ Id=19; Name='Microsoft PowerToys';    Cat='System';          Icon='[SYS]'; Admin=$true;  Cmd='$u=''https://api.github.com/repos/microsoft/PowerToys/releases/latest''; $a=(Invoke-RestMethod $u).assets | Where-Object { $_.name -match ''PowerToysSetup.*x64\.exe$'' } | Select-Object -First 1; $o="$env:TEMP\pt.exe"; Invoke-WebRequest $a.browser_download_url -OutFile $o; Start-Process $o -ArgumentList ''/install /quiet'' -Wait; Remove-Item $o -Force'; GitHub='https://github.com/microsoft/PowerToys';                   Desc='System utilities to maximize productivity. Includes ColorPicker, FancyZones, Run, etc.' }
    [PSCustomObject]@{ Id=20; Name='UniGetUI (WingetUI)';    Cat='System';          Icon='[SYS]'; Admin=$true;  Cmd='$o="$env:TEMP\UniGetUI.exe"; Invoke-WebRequest "https://github.com/Devolutions/UniGetUI/releases/latest/download/UniGetUI.Installer.exe" -OutFile $o; Start-Process $o -ArgumentList "/VERYSILENT /SUPPRESSMSGBOXES /NORESTART" -Wait; Remove-Item $o -Force'; GitHub='https://github.com/Devolutions/UniGetUI';                  Desc='Intuitive GUI for Winget, Scoop, Chocolatey, Pip, Npm, and .NET packages.' }
    [PSCustomObject]@{ Id=21; Name='Windhawk';               Cat='System';          Icon='[SYS]'; Admin=$true;  Cmd='$u=''https://api.github.com/repos/ramensoftware/windhawk/releases/latest''; $a=(Invoke-RestMethod $u).assets | Where-Object { $_.name -match ''\.exe$'' } | Select-Object -First 1; $o="$env:TEMP\windhawk.exe"; Invoke-WebRequest $a.browser_download_url -OutFile $o; Start-Process $o -ArgumentList ''/VERYSILENT /SUPPRESSMSGBOXES /NORESTART /SP-'' -Wait; Remove-Item $o -Force'; GitHub='https://github.com/ramensoftware/windhawk'; Desc='The customization marketplace for Windows and Windows programs.' }
    [PSCustomObject]@{ Id=22; Name='Firefox ESR';            Cat='System';          Icon='[SYS]'; Admin=$true;  Cmd='$o="$env:TEMP\FirefoxESR.exe"; Invoke-WebRequest "https://download.mozilla.org/?product=firefox-esr-latest-ssl&os=win64&lang=en-US" -OutFile $o; Start-Process $o -ArgumentList "/S" -Wait; Remove-Item $o -Force';                                         GitHub='https://www.mozilla.org/en-US/firefox/enterprise/';       Desc='Mozilla Firefox Extended Support Release (ESR) for maximum stability.' }
)
